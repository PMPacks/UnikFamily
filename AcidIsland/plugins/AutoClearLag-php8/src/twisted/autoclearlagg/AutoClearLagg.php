        ...
        $this->getScheduler()->scheduleRepeatingTask(new ClosureTask(function($_) : void{
            if(--$this->seconds === 0){
                ...
                foreach($this->getServer()->getLevels() as $level){
                    foreach($level->getEntities() as $entity){
                        if($this->clearItems && $entity instanceof ItemEntity){
                            ...
                            ++$entitiesCleared;
                        }else if($this->clearMobs && $entity instanceof Creature && $entity->getMaxHealth() < 30 &&!$entity instanceof Human){
                ...
                if($this->messages[self::LANG_ENTITIES_CLEARED] !== ""){
                   (String) $msg = $this->messages[self::LANG_ENTITIES_CLEARED];
                   foreach ($this->getServer()->getOnlinePlayers() as $p) {
                    $p->sendTip(str_replace((String)"{COUNT}", (String)$entitiesCleared, (String)$msg));
                }
            }

                $this->seconds = $this->interval;
            }else if(in_array($this->seconds, $this->broadcastTimes) && $this->messages[self::LANG_TIME_LEFT] !== ""){
                (String)$msg = $this->messages[self::LANG_TIME_LEFT];
                foreach ($this->getServer()->getOnlinePlayers() as $p) {
                $p->sendTip(str_replace((String)"{SECONDS}", (String)$this->seconds, (String)$msg));
            }
        }
        }), 20);
    }
