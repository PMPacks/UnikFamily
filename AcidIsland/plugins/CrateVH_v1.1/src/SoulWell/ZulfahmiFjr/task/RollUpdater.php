...
class RollUpdater extends Task{
      ...
      if($this->prediction === null){
       ...
       if($item !== null){
        if($p->isOnline()){
         ...
         $p->sendMessage("§l§c【 §fvιcтoʀʏ §eoғ §6ʟᴇԍᴇɴᴅ §c】§b Bạn nhận được §f".$item->getName()." §bvới số lương§e ".$item->getCount().".");
        }
       }
    ...
    public function getReward():?Item{
     ...
     if(!isset($reward["id"]) || !isset($reward["meta"]) || !isset($reward["amount"])){
      $this->pl->souls->set($p->getLowerCaseName(), $this->pl->souls->get($p->getLowerCaseName()) + 1);
      $this->pl->souls->save();
      $p->sendMessage("§l§c【 §fvιcтoʀʏ §eoғ §6ʟᴇԍᴇɴᴅ §c】§c Đã xảy ra lỗi khi đang quay crate lucky, key của bạn sẻ được trả lại.");
      return null;
     }
