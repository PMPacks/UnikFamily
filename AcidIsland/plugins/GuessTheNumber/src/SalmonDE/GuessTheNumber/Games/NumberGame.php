...
abstract class NumberGame {
    ...
    public function getAnnounceMessage(Main $plugin): string{
        ...
        $msg .= TF::GOLD.$plugin->getMessage('general.game.calculation', $this->getCalculation().TF::GOLD).TF::RESET."\n";
    ...
    public function checkAnswer(string $answer, Player $player, Main $plugin): bool{
        if($this->isSolution($answer)){
            ...
            if(!$event->isCancelled()){
                ...
                $player->sendMessage("§d§l●§e Bạn nhận được§6 100.000 Xu§e ở§a MINIGAMES ĐƯỜNG LÊN ĐỈNH OLYMPIA.");
                $plugin->stopGame();
                return true;
            }
        }
