...
namespace BaliGamerz\SkyBlock\libraries\xenialdan\apibossbar;
...
class BossBar
{
    ...
    {
        if ($entity instanceof Entity && ($entity->isClosed() || $entity->isFlaggedForDespawn())) throw new \InvalidArgumentException("Thực thể $entity không thể được sử dụng vì nó không còn hợp lệ nữa (bị đóng hoặc Encode)");
    ...
    {
        return __CLASS__ . " ID: $this->entityId, Players: " . count($this->players) . ", Tiêu đề: \"$this->title\", Phụ đề: \"$this->subTitle\", Phần trăm: \"".$this->getPercentage()."\"";
    }
