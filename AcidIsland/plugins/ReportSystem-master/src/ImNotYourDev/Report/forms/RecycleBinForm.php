...
class RecycleBinForm extends MenuForm
{
    ...
    {
        $title = "§e§lReport RecycleBin";
        $text = "Chọn một đơn tố cáo để xóa";
        ...
        parent::__construct($title, $text, $this->options, function (Player $player, $data) : void {
            if($data == 0){
                ...
                $player->sendMessage(Report::getInstance()->prefix . "Tất cả đơn tố cáo đã được xóa!");
                ...
                Report::getInstance()->deleteForEver($item["nestdir"]);
                $player->sendMessage(Report::getInstance()->prefix . "Đã xóa đơn tố cáo!");
                $player->sendForm(new $this);
            }
            return;
        });
